...
class PartitionRefinement:
    ...
    def __init__(self, n):
        ...
        c = (
            PartitionClass()
        )  # initially there is a single class of size n
        self.classes = c  # reference to first class in class list
        self.items = [
            PartitionItem(i, c) for i in range(n)
        ]  # value ordered list of items
            ...
            if 0 <= i < len(self.items):  # ignore if outside of domain
                ...
                if not c.split:  # possibly create new split class
                    ...
                    if self.classes is c:
                        self.classes = (
                            c.split
                        )  # always make self.classes point to the first class
                    has_split.append(c)
    ...
    def tolist(self):
        ...
        """
        return [
            [x.val for x in theclass.items] for theclass in self.classes
        ]
    ...
    def order(self):
        ...
        """
        return [
            x.val for theclass in self.classes for x in theclass.items
        ]
